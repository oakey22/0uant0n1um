...
// Quantonium: total coins staked (non-spendable until maturity)
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-Quantonium-address
...
// Quantonium: get current stake weight
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CQuantoniumAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CQuantoniumAddress(address).ToString());
}
...
// Quantonium: check 'spent' consistency between wallet and txindex
// Quantonium: fix wallet spent state according to txindex
...
// Quantonium: disable transaction (only for coinstake)
