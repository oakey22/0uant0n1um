...
{
    QString ret = QString("Quantonium:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non QNM unit input in lnReqAmount, we generate the URI with QNM as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(QuantoniumUnits::format(QuantoniumUnits::QNM, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
